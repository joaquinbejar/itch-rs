use itch_protocol::{enums::*, messages::*, primitives::*, Message};

/// Canonical synthetic session covering all 20 ITCH 5.0 message kinds.
///
/// This fixture is the default reference session for `itch-server`,
/// and is used by tests and fuzzers as a known-good corpus.
/// Every `Message` variant appears at least once, in natural session order.
///
/// Users can wrap this directly with `IteratorSource::new(canonical_session())`
/// to get a `MessageSource` (see issue #52).
///
/// # Session Structure
///
/// 1. Session lifecycle (`S` StartOfMessages)
/// 2. Reference data (`R` for 3 symbols: AAPL, MSFT, SPY)
/// 3. Market state transitions (`H` trading action, `Y` Reg SHO, `L` participant, `V`/`W` MWCB, `K` IPO)
/// 4. Market hours start (`S` StartOfMarketHours)
/// 5. Order lifecycle (`A`/`F` add, `E`/`C` execute, `X`/`D` cancel/delete, `U` replace)
/// 6. Trades (`P` non-cross, `Q` cross, `B` broken)
/// 7. Administrative (`I` NOII, `N` RPI)
/// 8. Session shutdown (`S` EndOfMarketHours → EndOfSystemHours → EndOfMessages)
pub fn canonical_session() -> Vec<Message> {
    ...
    let session = h(32_400_000_000_000, 0);
...
mod tests {
    ...
    fn canonical_session_covers_every_message_variant() {
        ...
        for variant in &expected {
            assert!(
                ...
                "canonical_session missing variant: {}",
                variant
            );
        }
    ...
    fn canonical_session_roundtrip() {
        ...
        for msg in &session {
            ...
            let decoded = Message::decode(&buf).expect("decode failed");
            assert_eq!(msg, &decoded, "roundtrip failed for {:?}", variant_of(msg));
        }
    }
}
