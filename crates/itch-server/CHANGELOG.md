...
- `canonical_session()` returns 24 messages exercising **all** 20
  ITCH 5.0 message kinds (`S`, `R`, `H`, `Y`, `L`, `V`, `W`, `K`,
  `A`, `F`, `E`, `C`, `X`, `D`, `U`, `P`, `Q`, `B`, `I`, `N`),
  verified by an exhaustive `variant_of(&Message)` match-based test.
...
- `ITCH_BIND` (default `127.0.0.1:9100`) and `ITCH_DELAY_MS`
  (default `0`) env vars to control the bind address and inter-
  message delay.
